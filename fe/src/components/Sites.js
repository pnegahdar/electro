...
const SiteCard = (props) =>
  <Card fluid style={{textAlign: "center"}}>
    <Card.Header>
      <h2 style={{margin: "5px"}}>{props.site.name}
      </h2>
    </Card.Header>
    <Card.Content style={{textAlign: 'left', fontSize: '16px'}}>
      <p><strong><Icon name="git"/>{ "Repo: "}</strong></p>
      <p style={{overflow: "hidden", textOverflow: "ellipsis"}}>{props.site.git_repo}</p>
      <p><strong><Icon name="fork"/>Branch: </strong>{props.site.branch}</p>
      <p><strong><Icon name="folder"/>Root: </strong>{props.site.root}</p>
      <p><strong><Icon name="heartbeat"/>Status: </strong>
        {props.site.healthy ?
      ...
      <Button.Group widths={8} vertical={true}>
        {_.map(props.site.hostnames, (hostname) =>
          <Button key={hostname} onClick={() => {
            window.location = `${window.location.protocol}//${hostname}`
          }} content={hostname} icon="globe"/>
        )}
  ...
  render = () =>
    <Grid>
      ...
      <Grid.Column computer={12} tablet={8} mobile={16}>
        <h2>Sites</h2>
        <Grid>
          {_.map(this.state.sites, (site, id) => <Grid.Column computer={8} tablet={16} mobile={16} key={id}>
