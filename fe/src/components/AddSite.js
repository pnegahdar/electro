...
const ValidatedInput = props => <div />;
...
export class AddSite extends React.Component {
    ...
    this.setState({ loading: true, errors: "", success: "" }, async () => {
      ...
      if (resp.error) {
        ...
        this.setState({
          errors: "",
          loading: false,
          success: `Site ${this.state.form
            .name} added with hostnames ${hostnameList} added. Please point CNAMES to ${this
            .props.wildCard} if not already on the wildcard.`
        });
      }
      this.props.reload();
    });
  ...
  splitAndCorrectHosts = value =>
    _.filter(_.map(value.split(/,\s?/), s => s.trim().toLowerCase()));

  validateHostnames = value =>
    _.every(
      _.map(this.splitAndCorrectHosts(value), v => !/[^a-zA-Z.-]/g.test(v))
    );
  ...
  render = () => {
    const suggestHostname =
      this.state.form.name &&
      `${this.state.form.name}.${this.props.wildCard}`
        .toLowerCase()
        .replace(/ /g, "-");
    return (
      <Form>
        ...
        <ValidateGroup impatientFeedback={true}>
          <Validate impatientFeedback={true} validators={[this.validateName]}>
            <Form.Input
              name="name"
              label="Name*"
              placeholder="My Favorite Project"
              onChange={this.handleChange}
            />
            <ErrorMessage>Name must be at least 3 characters.</ErrorMessage>
          </Validate>
          <Validate
            impatientFeedback={true}
            validators={[value => isGitUrl(value)]}
          >
            <Form.Input
              name="git_repo"
              label="Git URL*"
              placeholder="https://github.com/pnegahdar/electro.git"
              onChange={this.handleChange}
            />
            <ErrorMessage>
              A valid git url (https://projec.git, git@)
            </ErrorMessage>
          </Validate>
          <Validate
            impatientFeedback={true}
            validators={[this.validateHostnames]}
          >
            <Form.Input
              name="hostnames"
              label="Hostnames*"
              placeholder="apidocs.domain.com, docs.domain.com"
              onChange={this.handleChange}
            />
            {(suggestHostname &&
              <small>
                Suggested: <span>{suggestHostname}</span>
              </small>) ||
              <div />}
          ...
          <Button.Group style={{ marginBottom: "15px" }}>
            <Button
              icon="lock"
              active={this.state.showLoginForm}
              onClick={this.toggleLoginForm}
            />
            <Button
              icon="key"
              active={this.state.showKeyForm}
              onClick={() => this.toggleFormField("showKeyForm", "ssh_key")}
            />
            <Button
              icon="shield"
              active={this.state.showDeletePassForm}
              onClick={() =>
                this.toggleFormField("showDeletePassForm", "delete_password")}
            />
          </Button.Group>
